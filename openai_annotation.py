...
- `condition`: The health condition the person is using cannabis to address. Choose from: `sleep`, `pain`, `anxiety`, or `other`.
...
def annotate_file_with_gpt(input_data, output_csv_path, rate_limit=1.5):
    """
    Annotates Reddit comments with GPT and saves results to CSV.

    Parameters:
        input_data (str or pd.DataFrame): path to input CSV or DataFrame with a 'text' column
        output_csv_path (str): path to save the annotated CSV
        rate_limit (float): seconds to wait between API calls
    """
    if isinstance(input_data, pd.DataFrame):
        ...
        raise ValueError("input_data must be a pandas DataFrame or a CSV file path")

    texts = df['text'].dropna().tolist()

    #initialize CSV with header
    header = ["compound", "brand", "dosage", "method", "symptom", "outcome", "polarity", "false_positive", "text", "error"]
